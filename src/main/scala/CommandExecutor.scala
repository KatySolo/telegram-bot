...
//  }
//
//  def answer(userID: Long): Parser[String] = "/answer" ~> idx ~ anyWord ^^ { case i ~ ans => answer(i, ans) }
//
...
////  }
//
//  def create_poll(name: String, anon: Boolean, vis: Boolean, start: Option[Date], stop: Option[Date]): String = {
//    val poll = Poll(name, anon, vis, start, stop, id.next())
//    Polls += poll.id -> poll
//    s"Your poll id: ${poll.id}"
//  }
